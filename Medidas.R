...
# La moda de mi variable es 3700 y 3800
...
# Mi varianza es de 641436.2

# 2.- Cálculo de la desviación estándar
sd(BD$masa_corporal_g)
# Mi desviación estandar es 800.8971

# 3.- Error
media_masa<-mean(BD$masa_corporal_g)
media_masa
# La media de mi masa es 4202.253
...
CV
# Mi coeficiente de variacion es 19.05876

# 5.- Rango intercuartilico (IQR)
IQR(BD$masa_corporal_g)
# Mi rango intercuartilico es 1206.25
...
max(masa)
min(masa)
# Mi valor maximo (Q3) de masa es 6300 y 
# mi valor minimo (Q1) de masa es 2700
...
quintil
# Mis quintiles son 3475 3800 4300 4950
...
decil<-quantile(BD[["largo_aleta_mm"]], 
                p=c(.10, .20, .30, .40, .50, .60,
                    .70, .80, .90))
decil
# Mis deciles son 185 188 191 194 197 203 210 215 221 

# Percentil
percentil<-quantile(BD[["largo_aleta_mm"]], 
                    p=c(.33, .66, .99))
percentil
# Mis percentiles son 192 209 230 
